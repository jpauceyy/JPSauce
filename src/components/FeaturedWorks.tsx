        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 md:gap-8 lg:gap-16">
          {projects.map((project, i) => (
            <motion.div
              ...
              className={`flex flex-col group ${i === 0 ? 'md:col-span-7' :
                  i === 1 ? 'md:col-span-5' :
                    i === 2 ? 'md:col-span-12' :
                      i === 3 ? 'md:col-span-5' :
                        'md:col-span-7'
            ...
            >
              <Link to={`/work/${project.id}`} className="flex flex-col w-full h-full">
                {/* Image Container */}
                <div
                  className={`relative w-full mb-8 overflow-hidden bg-neutral-900 cursor-none ${i === 0 ? 'aspect-square md:aspect-[4/3]' :
                      i === 1 ? 'aspect-square md:aspect-square' :
                        i === 2 ? 'aspect-square md:aspect-[21/9]' :
                          i === 3 ? 'aspect-square md:aspect-square' :
                ...
                >
                  ...
                  <motion.img
                    ...
                    loading="lazy"
                    decoding="async"
                    className={`absolute inset-0 w-full h-full object-cover transition-all [transition-timing-function:cubic-bezier(0.16,1,0.3,1)] delay-[100ms] ${(project as any).hoverVideo
                  ...
                  {(project as any).hoverVideo && (
                    <motion.video
                      ...
                      playsInline
                      preload="none"
                      className={`absolute inset-0 w-full h-full object-cover transition-all duration-[1200ms] [transition-timing-function:cubic-bezier(0.16,1,0.3,1)] delay-[100ms] group-hover:scale-[1.08] ${hoveredProject === project.id ? 'opacity-100' : 'opacity-0'
                        }`}
                    />
                  )}
